...
function handleError(error: any, res: Response){
	...
	if (error.code === 'ECONNREFUSED'){
		console.error('ERROR: Cannot connect with Nu Server');
		res.status(503).send('ECONNREFUSED: Cannot connect with Nu Server');
		return;
	}
	...
	res.status(statusCode).send('ERROR: Cannot create Nu session');
}
